...
library(DESeq2)
```
**I load pheatmap and deseq2 for question 3**
...
```
**Okay, so the first question asks what fraction of the transcripts overlaps with the transcripts on the opposite side. I simply used bed tools for this, -S for checking opposite strands and -u for only counting each overlap once and I got a fraction of 0.08** 
...
**Next I plotted a distribution of the overlaps. Used -c to get the counts. As you can see from the histgram, most of the overlaps occur only once and then the number of overlaps drastically falls off**
...
**The transcript with the most overlaps was NR_134588.1. It had 81 overlap and you can see it in the genome browser picture. The transcript overlaps with the same gene 81 times, it's called CELF4. So there are two possible models to explain antisense transcriptions. One is that they silence each other. We see an overlap in introns, so silencing can only work if this occurs before splicing. However, if silencing occurs after splicing, this analysis does not pick that up. In order to improve this analysis we have to only check for overlapping exons. The secon model suggests that the RNA polymerase clashes, which would produceincomplete RNAs without any polyA tail or anything like that. So I'm not sure if sequencing techniques can pick up these RNAs. In order to improve the analysis for this case, we have to use a sequencing technique that can pick these truncated RNAs.**
...
** From the plot we can see basically 2things happening. On the sense strand there is a spike at around the 600 position, which is the transcription start site. This implies that these are normaly also destroyed by rpp40 and now they have an increase transcription since they are no longer degreaded. And for the antisense strand, we can see a huge spike around thr -100 position that goes all the way to the -600 position. If you look up RPP40 on uniprot, you'll see that it is known to degrade promoter-upstream transcripts, which is exactly what we're seeing here. There is a massive increase in transcripts up stream of the promoter which are normally destroyed. There is also a spike at the +300 position in the anti sense strand, these could be antisense RNA that regulate the sense RNA but are normally also degraded y rpp40 but now we can see all of this because rpp40 is silenced. **
...
** In order to ensure that files have the proper columns, I just use read.table**
...
pheatmap(top10_percent, show_rownames = F, annotation_col = column_to_rownames(studyDesign, "Sample"), scale="row")
pheatmap(top10_percent, show_rownames = F, annotation_col = column_to_rownames(studyDesign, "Sample"), scale="column")
```

** I created the heatmap with standard settings as requested and added the annotation column. You can immediately see something srange, Sample 6 clusters with the treatments even though it is a control, and Sample 18 clusters with the controls even though it is a treament. There also a subset of genes that are comparitively highly expressed for the treatment samples**
...
** I performed PCA and you can see that once again Sample 6 and 18 are swapped. Batch effects may also be present since the samples seem to be spread across PC2 in pairs and there is a cluster around the 0 point for both control and treatment **
...
** Since Sample 6and 18 are swapped in both plots, this suggests a mixup between the two samples. I've swapped the labels are performed PCA again and they appear in the correct positions**
...
** The uncorrected data was has way fewer up regulated and downregulated genes compared to the corrected data**
...
** The MA plot shows blue dots which are differentially expressed according to our selection criterai (the logFC and padj values), the red points are not differentially expressed and the graypoints which have too low of a count to perform calculations. We can see that the trendline for the pints does not match the zero line, which means there is a systematic bias in the data, and we have to perform quantile normalization to fix it**
...
```
phyper = 0.6811489
The chance of this drawing 31 diabtese genes is 0.68, so it could definitely happen by chance
